...
impl Component {
    ...
    pub(crate) async fn process_update_until_consistency(&mut self, update:Update) -> Vec<JoinHandle<Result<Response<ProcessUpdateResponse>, Status>>> {
        ...
        while !updates_to_process.is_empty() || !reprocess_intervals.is_empty() {
            ...
            match update {
                Update::Event(event) => {
                    info!("{} processing event with id {}", &self.query.name, &event.event_id);
                    if !retracted_ids.contains(&event.event_id) {
                        let (mut new_updates, mut new_handles) = self.process_new_event(&event).await;
                        updates_to_process.append(&mut new_updates);
                        handles.append(&mut new_handles);
                    } else {
                        info!("{} event with id {} was retracted, doing nothing", &self.query.name, &event.event_id);
                    }
                },
                Update::Deltas(deltas) => {
                    info!("{} processing deltas with id {} and event id {}", &self.query.name, &deltas.deltas_id, &deltas.origin_id);
                    if !retracted_ids.contains(&deltas.deltas_id) && !retracted_ids.contains(&deltas.origin_id) {
                        let mut new_reprocess_intervals = self.process_new_deltas(&deltas);
                        reprocess_intervals.append(&mut new_reprocess_intervals);
                    } else {
                        info!("{} deltas with id {} and event id {} was retracted, doing nothing",
                        &self.query.name, &deltas.deltas_id, &deltas.origin_id);
                    }
                }
                Update::Retractions(retractions) => {
                    info!("{} processing retractions with id {} ", &self.query.name, &retractions.retraction_id);
                    for id in &retractions.deltas_ids { retracted_ids.insert(id.clone());};
            ...
            if updates_to_process.is_empty() && !reprocess_intervals.is_empty() {
                let (mut new_updates, mut new_handles, new_reprocess_intervals) = self
    ...
    fn process_retractions(&mut self, retractions: Retractions) -> Vec<ReprocessInterval> {
        ...
        debug!("{} found {} deltas to retract for retraction {} with deltas id {:?}", &self.query.name, deltas_to_retract.len(), &retractions.retraction_id, &retractions.deltas_ids);
    ...
    fn process_new_deltas(&mut self, deltas: &Deltas) -> Vec<ReprocessInterval> {
        ...
        let existing_deltas_by_edge = self
            .store
            .get_deltas_by_edge_vec(new_deltas_edges_vec);
        let updated_deltas_by_edge = self.store.add_deltas_and_get_updated_deltas_by_edge(&deltas);
        info!("{} new deltas processing took {} μs", &self.query.name, now.elapsed().as_micros());
        self.update_edges_and_find_intervals_to_reprocess(existing_deltas_by_edge, updated_deltas_by_edge)
    }

    //Idempotent operation..
    async fn process_event(&mut self, event: &Event) -> (Vec<Update>, Vec<JoinHandle<Result<Response<ProcessUpdateResponse>, Status>>>) {
        let now = Instant::now();
        debug!("{} processing event {:?} with timestamp {:?}", &self.query.name, &event.event_id, &event.timestamp);
        ...
        let matches_by_hash = BTreeMap::from_iter(
            matches
                .into_iter()
                .map(|m| (m.stable_hash(self.config), m)),
        );
        let existing_output_hashes_by_match_hashes = self.store.get_event_output_hash_by_match_hash(&event.event_id);
        let existing_match_hashes_by_output_hashes = BTreeMap::from_iter(existing_output_hashes_by_match_hashes.iter().filter(|(_m,o)| o.is_some()).map(|(m,o)|(o.unwrap(), m.clone())));
        ...
        for (match_hash, grouped_match) in matches_by_hash.iter() {
            if !existing_output_hashes_by_match_hashes.contains_key(match_hash) {
                debug!("{} found new match {} for event {}, computing output", &self.query.name, match_hash, &event.event_id);
                let new_deltas_opt = self
                    .process_new_match(grouped_match, event)
                    .await;
                if let Some(new_deltas) = new_deltas_opt {
                    let new_output_hash = new_deltas.stable_hash(self.config);
                    new_output_hashes_by_match_hashes.insert(match_hash.clone(), Some(new_output_hash));
                    if let Some(old_match_hash) = existing_match_hashes_by_output_hashes.get(&new_output_hash) {
                        debug!("{} output already exists for new match {} event {}", &self.query.name, match_hash, &event.event_id);
                        //We will only relink the store so that the new match is linked to the old update
                        self.store.replace_old_match_with_equivalent_new_match(&event.event_id, old_match_hash, match_hash);
                        let removed = matches_to_possibly_retract.remove(&old_match_hash);
                        //Else we have a duplicate match
                        assert!(removed);
                    } else {
                        debug!("{} output did not exist for new match {} event {}, creating update", &self.query.name, match_hash, &event.event_id);
                        new_output_hashes_by_match_hashes.insert(match_hash.clone(), None);
                        let (new_update_opt, mut handles) = self.create_and_send_deltas_update(new_deltas, &event.event_id, match_hash).await;
        ...
        if !matches_to_possibly_retract.is_empty() || !new_output_hashes_by_match_hashes.is_empty() {
            ...
            self.store.update_matches(&event.event_id, updated_output_hashes_by_match_hashes);
        }

        if !matches_to_possibly_retract.is_empty() {
            debug!("{} found {} matches to retract", &self.query.name, matches_to_possibly_retract.len());
        ...
        info!("{} event processing took {} μs", &self.query.name, now.elapsed().as_micros());
    ...
    fn retract_deltas(&mut self, deltas: Vec<DeltaAndDeltasId>) -> Vec<ReprocessInterval> {
        ...
        debug!("{} retract deltas by edge {:?}", &self.query.name, &retract_deltas_by_edge);
        let existing_deltas_by_edge = self
            .store
            .get_deltas_by_edge_vec(retract_deltas_by_edge_vec);
        debug!("{} existing deltas by edge {:?}", &self.query.name, &existing_deltas_by_edge);
        let updated_deltas_by_edge = self.store.remove_deltas_by_edges_and_get_updated(retract_deltas_by_edge);
        debug!("{} updated deltas by edge {:?}", &self.query.name, &updated_deltas_by_edge);
        self.update_edges_and_find_intervals_to_reprocess(existing_deltas_by_edge, updated_deltas_by_edge)
    }

    fn update_edges_and_find_intervals_to_reprocess(&mut self, mut existing_deltas_by_edge: BTreeMap<Edge, BTreeSet<DeltaAndDeltasId>>, updated_deltas_by_edge: BTreeMap<Edge, BTreeSet<DeltaAndDeltasId>>) -> Vec<ReprocessInterval> {
        ...
        for (e, updated_deltas_and_deltas_ids) in updated_deltas_by_edge {
            ...
            let existing_deltas_set = BTreeSet::from_iter(existing_deltas_and_deltas_ids.into_iter().map(|d| d.delta));
            let existing_deltas: Vec<Delta> = existing_deltas_set.into_iter().collect();

            let updated_deltas_set = BTreeSet::from_iter(updated_deltas_and_deltas_ids.into_iter().map(|d| d.delta));
            let updated_deltas: Vec<Delta> = updated_deltas_set.into_iter().collect();

            let existing_edges_with_timestamp = edges_from_deltas(&existing_deltas.iter().collect());
            let remaining_edges_with_timestamp = edges_from_deltas(&updated_deltas.iter().collect());
    ...
    async fn reprocess_events_in_intervals_until_internal_update(
    ...
    ) -> (Vec<Update>, Vec<JoinHandle<Result<Response<ProcessUpdateResponse>, Status>>>, Vec<ReprocessInterval>) {
        let now = Instant::now();
        ...
        'outer: for i in 0..non_redundant_intervals_to_reprocess.len() {
            ...
            let event_ids_in_interval = self.store.get_event_ids_in_interval(interval.from, interval.to);
            for j in 0..event_ids_in_interval.len() {
                ...
                if let Some(event) = event_opt {
                    ...
                    let (mut new_internal_updates, mut new_handles) = self.process_event(&event)
                    .await;
                    ...
                    if new_internal_updates.len() > 0 {
                        ...
                        if !(interval.to.is_some() && interval.to.unwrap() < (event.timestamp + 1)) {
                            remaining_intervals.push(ReprocessInterval {from:event.timestamp, to:interval.to.clone()})
                        }
                        if i + 1 < non_redundant_intervals_to_reprocess.len() {
                            for k in (i+1)..non_redundant_intervals_to_reprocess.len() {
                                remaining_intervals.push(non_redundant_intervals_to_reprocess.get(k).unwrap().clone());
                            }
                        }
                        break 'outer;
                    }
        ...
        info!("{} reprocess events took {} μs", &self.query.name, now.elapsed().as_micros());
        (internal_updates, all_handles, remaining_intervals)
    }
    ...
    async fn process_new_match(
        ...
        debug!("{} processing new match for event {}", &self.query.name, &event.event_id);
        let deltas_opt = self
            .caller
    ...
    async fn create_and_send_deltas_update(&mut self, deltas: Deltas, event_id:&str, match_hash: &u64) -> (Option<Update>, Vec<JoinHandle<Result<Response<ProcessUpdateResponse>, Status>>>) {
        let (my_internal_update, cascaded_query_and_update_id, handles) = self
            .router
            .route_deltas_update(deltas)
            .await;
    ...
    async fn retract_matches(
        ...
        event_id: &str,
        event_timestamp: &u64,
    ) -> (Vec<Update>, Vec<JoinHandle<Result<Response<ProcessUpdateResponse>, Status>>>) {
        debug!("{} retracting {} matches for event {}", &self.query.name, matches_hashes.len(), event_id);
        let mut all_topic_names_and_deltas_ids = vec![];
        for m in matches_hashes {
            let topic_names_and_deltas_ids_opt =
                self.store.pop_deltas_ids_for_event_id_and_match_hash(event_id, m);
        ...
        let (internal_updates, handles) = self
            .router
            .route_retractions(
                all_topic_names_and_deltas_ids,
                event_timestamp,
            )
            .await;
    ...
    async fn process_new_event(&mut self, event: &Event) -> (Vec<Update>, Vec<JoinHandle<Result<Response<ProcessUpdateResponse>, Status>>>) {
...
fn get_deltas_and_deltas_id_by_edge(deltas: Vec<DeltaAndDeltasId>) -> BTreeMap<Edge, BTreeSet<DeltaAndDeltasId>> {
        let mut deltas_by_edge = BTreeMap::new();
        for d in deltas {
