...
for c in cluster_sizes:
    for s in scenario_sizes:
        ...
        testing_job[0]['spec']['template']['spec']['containers'][0]['command'] = ["/bin/bash", "-c",
                                                              ...
                                                              "/usr/local/bin/mbei-testdata-file-producer -i=msgs.yaml -u=/etc/config/names-url-map.yaml -l true"]
        ...
        wait_for_benchmark_cmd = ["kubectl", "wait", "job.batch/perftest-job", "--for=condition=complete", "--timeout=6000s"]
